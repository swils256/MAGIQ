...
\begin{empheq}[box=\fbox]{align}
	\label{eq:voxel_conc}
	[M]_\text{voxel} = & ~ \left(\frac{\sum_k^K A_k^m}{N_\text{avg}^{m} ~ \rho^m ~ G^m \left(\frac{f_{GM}}{f_{GM}+f_{WM}} R_{WM}^{m} + \frac{f_{GM}}{f_{GM}+f_{WM}}  R_{WM}^{m}\right)}\right) \nonumber \\
...
\begin{empheq}[box=\fbox]{align}
	[M]_\text{tissue} = & ~ \left(\frac{\sum_k^K A_k^m}{A_W}\right) \times \left(\frac{f_{GM} \alpha_{GM}^{W} R_{GM}^{W} + f_{WM} \alpha_{WM}^{W} R_{WM}^{W} + f_{CSF} \alpha_{CSF}^{W}  R_{CSF}^{W}}{{f_{GM}} R_{WM}^{m} + {f_{GM}}  R_{WM}^{m}}\right) \nonumber \\

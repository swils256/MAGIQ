...
int endianCheck_file(FILE *in_file, Endian_Check *endian_check, bool *swap_bytes
    ...
    // At this point I have no way of checking the file so the assumption is that
    // the file is little endian and is set so.
    // If output file is incorect, apply FORCED byte-swap.
    // If some method of detection will be possible, than the detection from 
    // ge2fitman routine can also be applied here.
                                                         
     if (verbose){
        printf("\n*The input data file:\n  \"%s\"\n",filename);  
        printf("*is assumed to have 'Little-Endian' structure.\n");
    }
    endian_check->fileStruct = LITTLE_END;    
    
    if(endian_check->systemStruct == BIG_END){ 
    	// Need swap byte...system and file doesn't match.
        if (verbose){
        	printf("  Byte swapping applied.\n");
        }
     	*swap_bytes = true;  
    }else{          
        // No need for byte swaping...system and file match.
        if (verbose){
...
/******************************************************************************
       "Not-a-number" checking routine.
       Return: 1 - float value is not a number
               0 - otherwise
******************************************************************************/
int isNAN(float float_value){
    ...
    // I used this instead the flush routine, since gcc is not friendly with
    // the flush routine
    do{
    	junk=getc(stdin);                       
    }while(junk!='\n');
    */
    ...
    sprintf(temp_float_string, "%f", float_value); 
    if (strcmp(temp_float_string,"-NaN")==0 || 
        strcmp(temp_float_string,"NaN")==0 || 
        strcmp(temp_float_string,"Inf")==0 || 
        strcmp(temp_float_string,"-Inf")==0){
	return 1;
   }else{
   	return 0;
...
void disp_help(int version){
    ...
    if (version == HELP_SHORT) { // Short version.
        printf("\nUsage:\n");
        printf(">> sim2fitman [-options] in_file [-in_file_options] out_file\n\n");
	...
	//revised by jd to suit Siemens one data set
	//printf("       -(ecc /or/ ...\n");
        ...
        printf(">> sim2fitman [-options] in_file [-in_file_options] out_file\n\n");
        //printf("             [reference_file][-ref_file_options]out_file\n\n");
        //printf("\nUsage:\n>> sim2fitman [-options] in_file [-in_file_options][reference_file][-ref_file_options]out_file\n\n");
        
        printf("   in_file          Input suppressed data file.\n");
        //printf("   reference_file   Input unsuppressed data file.\n");       
        printf("   out_file         Output suppressed or unsuppressed datafile name.\n");
	...
	// printf("          should be set to obtain valid data:\n");
        ...
        //printf("             -A reference file must also be specified.\n\n");       
